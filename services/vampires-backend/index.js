...
const classroomRoom = (classroomId) => `classroom:${classroomId}`;
const classroomTeacherRoom = (classroomId) => `classroom:${classroomId}:teachers`;

function getClassroomSession(classroomId, teacherId) {
  let session = classroomSessions.get(classroomId);
  ...
  if (!session) {
    ...
    classroomSessions.set(classroomId, session);
  }
...
function getClassroomStudents(classroomId) {
  return Array.from(classroomPresence.get(classroomId)?.values() || []);
}

function serializeClassroomSession(classroomId) {
  ...
  return {
    ...
    connectedStudents: connectedStudents.map(student => ({
      id: student.id,
      name: student.name,
      assignedGameCode: session?.assignments.get(student.id) || null,
      joinedAt: student.joinedAt,
    })),
    previewGroups: (session?.previewGroups || []).map((studentIds, index) => ({
      groupNumber: index + 1,
    ...
    games: (session?.gameCodes || []).map((code, index) => {
...
function emitClassroomState(classroomId) {
  if (!classroomId) return;
  io.to(classroomTeacherRoom(classroomId)).emit(
    'classroom_session_state',
    serializeClassroomSession(classroomId)
  );
...
function sanitizeSettings(settings = {}) {
  ...
  return {
    discussionTime: clampInteger(settings.discussionTime, 120, 10, 600),
    ...
    voiceInputMode: settings.voiceInputMode === 'voice-activity' ? 'voice-activity' : 'push-to-talk',
    ttsProvider: settings.ttsProvider === 'elevenlabs' ? 'elevenlabs' : 'google',
    sttProvider: settings.sttProvider === 'google' ? 'google' : 'deepgram',
    elevenlabsModel: String(settings.elevenlabsModel || 'eleven_turbo_v2_5'),
    npcNationality: settings.npcNationality === 'lithuanian' ? 'lithuanian' : 'english',
    theme: ['day', 'christmas'].includes(settings.theme) ? settings.theme : 'dark',
    npcAllowedRoles: settings.npcAllowedRoles && typeof settings.npcAllowedRoles === 'object'
...
function sanitizeRoleConfig(roleConfig = {}) {
  const roles = ['Investigator', 'Lookout', 'Doctor', 'Jailor', 'Vampire', 'Vampire Framer', 'Jester'];
  const sanitized = { useDefault: roleConfig.useDefault !== false };

  roles.forEach(role => {
    const count = Number.parseInt(roleConfig[role], 10);
    sanitized[role] = Number.isFinite(count) ? Math.min(50, Math.max(0, count)) : 0;
  });
...
async function verifyTeacherClassroom(socket, classroomId) {
  if (socket.data.profile.role !== 'teacher') return null;
  ...
  if (error) {
    console.error('[Classroom] Failed to verify teacher classroom:', error);
  }
...
async function getActiveStudentClassroom(studentId) {
  ...
  if (error) {
    console.error('[Classroom] Failed to load active enrollment:', error);
  }
...
function attachStudentToAssignedGame(socket, session) {
  const studentId = socket.data.user.id;
  const code = session.assignments.get(studentId);
  const game = code ? games[code] : null;
  if (!game) return false;

  const player = game.addPlayer(studentId, socket.data.displayName, socket.id);
  socket.join(code);
  socket.emit('game_assigned', {
    code,
    playerId: studentId,
  ...
  if (game.state !== 'LOBBY' && player.role) {
...
function clearClassroomGames(session, notifyStudents = true) {
  session.generation += 1;
  session.gameCodes.forEach(code => {
    const game = games[code];
    if (!game) return;
    if (game.interval) clearInterval(game.interval);
    if (game.state !== 'GAME_OVER') game.endGame('Teacher Ended');
    delete games[code];
  });

  session.status = 'waiting';
  session.previewGroups = [];
  session.gameCodes = [];
  session.assignments.clear();

  if (notifyStudents) {
    io.to(classroomRoom(session.classroomId)).emit('classroom_session_reset');
  }
  emitClassroomState(session.classroomId);
}
...
class Game {
  ...
  async triggerNPCChatResponse(npc, isAddressed = false, isProactive = false) {
    ...
    try {
      ...
      if (message && message !== 'SILENCE' && this.state === 'DAY_DISCUSS') {
        ...
        // Generate TTS audio for the human players in this game.
        ...
        if (this.settings.enableTTS && ttsCtrl) {
          ...
          ttsCtrl.synthesizeSpeech(message, npc.id, this.settings.npcNationality || 'english', ttsOptions)
            .then(audioBase64 => {
              if (audioBase64) {
                io.to(this.code).emit('tts_audio', {
                  audio: audioBase64,
                  senderName: npc.name,
                  senderId: npc.id
                });
                console.log(`[Game] Sent TTS audio to players for ${npc.name}`);
              }
            })
  ...
  startTimer(seconds, callback) {
    ...
    this.interval = setInterval(() => {
      ...
      emitClassroomState(this.classroomId);
    }, 1000);
  }
...
io.on('connection', async (socket) => {
  ...
  if (identity.profile.role === 'student') {
    ...
    let presence = classroomPresence.get(classroomId);
    if (!presence) {
      presence = new Map();
      classroomPresence.set(classroomId, presence);
    }

    const previous = presence.get(identity.user.id);
    if (previous?.socketId && previous.socketId !== socket.id) {
      io.sockets.sockets.get(previous.socketId)?.emit(
        'session_replaced',
        'The game was opened in another tab.'
      );
    }
    ...
    emitClassroomState(classroomId);
  }
  ...
  socket.on('teacher_watch_classroom', async ({ classroomId } = {}) => {
    ...
    if (socket.data.classroomId && socket.data.classroomId !== classroomId) {
      ...
      socket.leave(classroomTeacherRoom(socket.data.classroomId));
    }
  ...
  socket.on('prepare_classroom_games', async ({
    ...
    const session = getClassroomSession(classroomId, socket.data.user.id);
    if (session.status === 'running') {
      socket.emit('classroom_error', 'End the current games before preparing another round.');
      return;
    }

    try {
      const connectedStudents = getClassroomStudents(classroomId);
      const groups = splitStudentsIntoGames(connectedStudents, Number(targetSize));
      session.targetSize = Number(targetSize);
      session.settings = sanitizeSettings(settings);
      session.roleConfig = sanitizeRoleConfig(roleConfig);
      session.npcCount = Math.min(20, Math.max(0, Number.parseInt(npcCount, 10) || 0));
      session.previewGroups = groups.map(group => group.map(student => student.id));
      emitClassroomState(classroomId);
    } catch (error) {
      socket.emit('classroom_error', error.message);
    }
  });

  socket.on('start_classroom_games', async ({ classroomId } = {}) => {
    const classroom = await verifyTeacherClassroom(socket, classroomId);
    if (!classroom) {
      socket.emit('classroom_error', 'You do not own this classroom.');
      return;
    ...
    const session = getClassroomSession(classroomId, socket.data.user.id);
    if (session.status === 'running') {
      socket.emit('classroom_error', 'Classroom games are already running.');
      return;
    }

    if (!session.previewGroups.length || !session.settings || !session.roleConfig) {
      socket.emit('classroom_error', 'Prepare and preview the groups before starting.');
      return;
    }

    const presence = classroomPresence.get(classroomId) || new Map();
    const missingStudent = session.previewGroups
      .flat()
      .find(studentId => !presence.has(studentId));

    if (missingStudent) {
      socket.emit('classroom_error', 'The connected student list changed. Prepare the groups again.');
      return;
    }
    ...
    for (let index = 0; index < session.previewGroups.length; index += 1) {
      const code = generateGameCode();
      const gameSettings = {
        ...session.settings,
        roleConfig: session.roleConfig,
      };
      const game = new Game(code, socket.data.user.id, gameSettings);
      game.classroomId = classroomId;
      game.groupNumber = index + 1;
      games[code] = game;
      session.gameCodes.push(code);

      session.previewGroups[index].forEach(studentId => {
        const student = presence.get(studentId);
        game.addPlayer(studentId, student.name, student.socketId);
        session.assignments.set(studentId, code);
        const studentSocket = io.sockets.sockets.get(student.socketId);
        if (studentSocket) {
          studentSocket.join(code);
        }
      });

      for (let npcIndex = 0; npcIndex < session.npcCount; npcIndex += 1) {
        await game.addNPC();
        if (session.generation !== startGeneration || games[code] !== game) {
          return;
        }
      }
      ...
      session.previewGroups[index].forEach(studentId => {
        const student = presence.get(studentId);
        const studentSocket = student ? io.sockets.sockets.get(student.socketId) : null;
        if (studentSocket) {
          studentSocket.emit('game_assigned', {
            code,
            playerId: studentId,
            groupNumber: index + 1,
            theme: session.settings.theme || 'dark',
      ...
      game.players.forEach(player => {
        if (player.socketId) {
          io.to(player.socketId).emit('role_info', {
            role: player.role,
  ...
  socket.on('teacher_skip_game_timer', async ({ classroomId, code } = {}) => {
    const classroom = await verifyTeacherClassroom(socket, classroomId);
    const session = classroom ? classroomSessions.get(classroomId) : null;
    if (!classroom || !session?.gameCodes.includes(code) || !games[code]) {
      socket.emit('classroom_error', 'Game control is not available.');
      return;
    }
    games[code].skipTimer();
    emitClassroomState(classroomId);
  });

  socket.on('teacher_end_game', async ({ classroomId, code } = {}) => {
    const classroom = await verifyTeacherClassroom(socket, classroomId);
    const session = classroom ? classroomSessions.get(classroomId) : null;
    if (!classroom || !session?.gameCodes.includes(code) || !games[code]) {
      socket.emit('classroom_error', 'Game control is not available.');
    ...
    games[code].endGame('Teacher Ended');
    emitClassroomState(classroomId);
  });

  socket.on('teacher_end_all_games', async ({ classroomId } = {}) => {
    const classroom = await verifyTeacherClassroom(socket, classroomId);
    const session = classroom ? classroomSessions.get(classroomId) : null;
    if (!classroom || !session) {
      socket.emit('classroom_error', 'Classroom session is not available.');
      return;
    }
    clearClassroomGames(session);
  });
  ...
  socket.on('rejoin_game', ({ code, playerId }) => {
    const game = games[code];
    const session = game?.classroomId ? classroomSessions.get(game.classroomId) : null;
    const isAssignedStudent = socket.data.profile.role === 'student'
      && socket.data.user.id === playerId
      && session?.assignments.get(playerId) === code;

    if (game && isAssignedStudent) {
  ...
  socket.on('create_game', () => {
    socket.emit('error', 'Public game creation is disabled.');
  });

  // --- JOIN ---
  socket.on('join_game', () => {
    socket.emit('error', 'Public room joining is disabled.');
  });
  ...
  socket.on('get_stt_available', () => {
    socket.emit(
      'stt_available',
      deepgramSTTController.isAvailable() || googleSTTController.isAvailable()
    );
  });

  socket.on('disconnect', () => {
    const classroomId = socket.data.classroomId;
    if (!classroomId || socket.data.profile.role !== 'student') return;

    const presence = classroomPresence.get(classroomId);
    const currentPresence = presence?.get(socket.data.user.id);
    if (currentPresence?.socketId === socket.id) {
      presence.delete(socket.data.user.id);
      if (presence.size === 0) classroomPresence.delete(classroomId);
    }

    const session = classroomSessions.get(classroomId);
    const code = session?.assignments.get(socket.data.user.id);
    const player = code ? games[code]?.players.find(item => item.id === socket.data.user.id) : null;
