-- Avoid rewriting the active enrollment when accepting an invite for the
-- classroom that is already active.
...
BEGIN
  ...
  SELECT enrollment.is_active_classroom
  INTO v_target_is_active
  FROM enrollments AS enrollment
  WHERE enrollment.student_id = v_student_id
    AND enrollment.classroom_id = v_classroom_id
  FOR UPDATE;
